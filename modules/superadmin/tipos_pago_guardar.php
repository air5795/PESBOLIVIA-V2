...
if (!in_array($tipo, ['qr', 'transferencia', 'otro'])) $errores[] = "Tipo no válido";
...
if ($id > 0) {
    ...
    // Crear
    $query = "INSERT INTO tipos_pago (nombre, tipo, descripcion, instrucciones, imagen_qr, 
              numero_cuenta, banco, titular, ci_titular, estado) 
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($conexion, $query);
    mysqli_stmt_bind_param($stmt, "ssssssssss", $nombre, $tipo, $descripcion, $instrucciones, 
                          $qr_path, $numero_cuenta, $banco, $titular, $ci_titular, $estado);
    
    if (mysqli_stmt_execute($stmt)) {
        $nuevo_id = mysqli_insert_id($conexion);
        Session::registrar_actividad(Session::get_user_id(), 'crear', 'tipos_pago', $nuevo_id, "Tipo de pago creado: $nombre");
        $_SESSION['success'] = "Tipo de pago creado correctamente";
    } else {
        $_SESSION['error'] = "Error al crear el tipo de pago";
    }
}
