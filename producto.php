...
<head>
    ...
    <style>
        ...
        * {
        ...
        .modern-brand .li-yellow {
            color: #f1c40f;
            font-weight: 900;
            font-size: 0.95rem;
        }

        .modern-brand .via-green {
            color: var(--primary-color);
            font-weight: 900;
            font-size: 0.95rem;
        }
        ...
        .modern-subnav::-webkit-scrollbar {
            display: none;
        }
        ...
        .product-img-wrapper:hover .zoom-icon {
            opacity: 1;
        }
        ...
        .payment-option:hover {
            border-color: var(--primary-color);
            background: rgba(39, 204, 160, 0.1);
        }
...
<body>
        ...
        <div class="product-image-col">
            <?php if (!empty($imagenes_producto)): ?>
                ...
                foreach ($imagenes_producto as $img_item) {
                    if ($img_item['es_principal']) {
                        $img_principal = $img_item;
                        break;
                    }
                }
                ?>
                <div class="product-img-wrapper" onclick="abrirLightbox(0)">
                    <img src="<?php echo BASE_URL . '/' . $img_principal['imagen']; ?>"
                        alt="<?php echo htmlspecialchars($producto['nombre']); ?>" class="product-main-img"
                        id="mainProductImg">
            ...
            <?php endif; ?>
        </div> <!-- Cierre de product-image-col (Columna Izquierda) -->

        <!-- Columna Derecha Unificada (Detalles + Compra) -->
        <div class="product-details-col">
            <!-- Cabecera Compacta en 2 Columnas -->
            <div class="d-flex justify-content-between align-items-start gap-3 flex-wrap mb-4" style="border-bottom: 1px solid var(--border); padding-bottom: 20px;">
                <!-- Columna Izquierda: Título + Categoría + Rating -->
                <div style="flex: 1; min-width: 250px;">
                    <h1 class="product-title" style="margin-bottom: 4px; font-size: 1.7rem; font-weight: 800;"><?php echo htmlspecialchars($producto['nombre']); ?></h1>
                    
                    <div class="d-flex align-items-center gap-3 flex-wrap">
                        <a href="tienda.php?categoria=<?php echo $producto['id_categoria']; ?>" class="product-category" style="margin-bottom: 0; font-size: 0.85rem;">
                            <i class="fas fa-folder me-1"></i> <?php echo $producto['categoria_nombre'] ?? 'PES Bolivia'; ?>
                        </a>
                        
                        <div class="product-rating" style="cursor: pointer; margin-bottom: 0; padding-bottom: 0; border-bottom: none; gap: 4px;" onclick="document.getElementById('resenas-section').scrollIntoView({behavior: 'smooth'})">
                            <?php
                            ...
                            for ($i = 0; $i < $entero; $i++)
                                echo '<i class="fas fa-star stars-filled" style="font-size: 0.85rem;"></i>';
                            if ($mitad)
                                echo '<i class="fas fa-star-half-alt stars-filled" style="font-size: 0.85rem;"></i>';
                            for ($i = 0; $i < $vacio; $i++)
                                echo '<i class="far fa-star stars-empty" style="font-size: 0.85rem;"></i>';
                            ?>
                            <span class="rating-text" style="font-size: 0.85rem; font-weight: 700; margin-left: 4px;"><?php echo number_format($rating, 1); ?></span>
                            <span class="rating-count" style="font-size: 0.8rem; color: var(--text-muted);">(<?php echo $total_resenas; ?>)</span>
                        </div>
                    </div>
                </div>
                
                <!-- Columna Derecha: Bloque de Precio Moderno con Estilo Premium -->
                <div class="text-center d-flex justify-content-center align-items-center" style="min-width: 170px; margin-left: auto; margin-right: auto; md-margin-left: 0;">
                    <?php if ((isset($producto['es_gratuito']) && strtolower($producto['es_gratuito']) === 'si') || empty($producto['precio']) || $producto['precio'] == 0): ?>
                        <div style="background: rgba(39, 204, 160, 0.08); border: 1px dashed var(--primary-color); border-radius: 12px; padding: 8px 16px; display: inline-block; text-align: center;">
                            <div class="price-label" style="font-size: 0.75rem; text-transform: uppercase; letter-spacing: 1px; color: var(--text-secondary); margin-bottom: 2px; font-weight: 600;">Estado</div>
                            <div class="price-free" style="font-size: 1.8rem; font-weight: 900; line-height: 1; color: var(--primary-color); text-shadow: 0 0 10px rgba(39,204,160,0.3); margin-bottom: 0;">GRATUITO</div>
                        </div>
                    <?php else:
                        $montoParts = explode('.', number_format($producto['precio'], 2));
                        $precio_usd = $producto['precio'] / floatval($CONFIG['tasa_cambio_usd'] ?? 6.96);
                    ?>
                        <div style="background: #141420; border: 1px solid var(--border); border-radius: 14px; padding: 10px 18px; display: inline-block; box-shadow: 0 4px 15px rgba(0,0,0,0.25); text-align: center; min-width: 170px;">
                            <div class="price-label" style="font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.8px; color: var(--text-secondary); margin-bottom: 3px; font-weight: 600;">Precio Oficial</div>
                            <div style="display: flex; align-items: baseline; justify-content: center; gap: 2px; color: var(--primary-color);">
                                <span style="font-size: 1rem; font-weight: 700; color: var(--primary-color); opacity: 0.95; margin-right: 1px;">Bs</span>
                                <span style="font-size: 2.1rem; font-weight: 900; line-height: 1; letter-spacing: -0.5px;"><?php echo $montoParts[0]; ?></span>
                                <span style="font-size: 1.1rem; font-weight: 800; opacity: 0.9;">.<?php echo $montoParts[1]; ?></span>
                            </div>
                            <div style="font-size: 0.75rem; font-weight: 600; color: #b5b5c5; margin-top: 4px; display: flex; align-items: center; justify-content: center; gap: 4px; border-top: 1px solid rgba(255,255,255,0.06); padding-top: 4px; white-space: nowrap;">
                                <i class="fas fa-globe-americas" style="font-size: 0.75rem; color: #b5b5c5; opacity: 0.8;"></i>
                                <span>Fuera de Bolivia: $<?php echo number_format($precio_usd, 2); ?> USD</span>
                            </div>
                        </div>
                    <?php endif; ?>
                </div>
            ...
            <div class="payment-purchase-section mb-4">
                <?php if ((isset($producto['es_gratuito']) && strtolower($producto['es_gratuito']) === 'si') || empty($producto['precio']) || $producto['precio'] == 0):
                    $link_descarga = $producto['link_descarga_directa'] ?? $producto['drive_link'] ?? '#';
                    ?>
                    <a href="<?php echo $link_descarga; ?>" target="_blank" class="btn-buy btn-free-modern w-100">
                        <i class="fas fa-download me-2"></i> Descargar Ahora
                    </a>
                <?php else: ?>
                    <?php if (!$is_logged): ?>
                        <a href="login.php" class="btn-buy btn-primary-modern w-100">
                            <i class="fas fa-lock me-2"></i> Iniciar Sesión para Comprar
                        </a>
                    <?php elseif ($ya_comprado): ?>
                        <div class="alert alert-success p-3 mb-3 w-100" style="border-radius: 12px; font-size: 0.9rem;">
                            <i class="fas fa-check-circle me-2"></i> Ya compraste este producto
                        </div>
                        <a href="modules/comprador/mis_compras.php" class="btn-buy btn-outline-modern w-100">
                    ...
                    <?php else: ?>
                        <form method="POST" action="procesar_compra.php" enctype="multipart/form-data" id="formCompra"
                            ...
                            <div class="payment-title"><i class="fas fa-credit-card me-2 text-primary"></i>Selecciona método de
                                pago:</div>

                            <?php if (mysqli_num_rows($tipos_pago) > 0): ?>
                                <div class="payment-list mb-3" style="display: flex; flex-direction: column; gap: 10px;">
                                    <?php while ($tipo = mysqli_fetch_assoc($tipos_pago)): ?>
                                        <div class="payment-option"
                                            onclick="selectMetodo(<?php echo $tipo['id']; ?>, '<?php echo !empty($tipo['imagen_qr']) ? BASE_URL . '/' . $tipo['imagen_qr'] : ''; ?>')"
                                            style="margin-bottom: 0;">
                                            <input type="radio" name="temp_pago" style="margin: 0;">
                                            <div>
                                                <div class="payment-name"><i
                                                        class="fas fa-money-bill-wave text-success me-2"></i><?php echo $tipo['nombre']; ?>
                                                </div>
                                                <?php if ($tipo['tipo'] === 'transferencia' && !empty($tipo['banco'])): ?>
                                                    <div class="payment-details">
                                                        <?php echo $tipo['banco']; ?>                    <?php if (!empty($tipo['numero_cuenta'])): ?> -
                                                            <?php echo $tipo['numero_cuenta']; ?>                    <?php endif; ?></div>
                                                <?php endif; ?>
                                            </div>
                                        </div>
                                    <?php endwhile; ?>
                            ...
                            <div id="comprobanteSection" class="comprobante-section text-center">
                                ...
                                <input type="file" class="form-control mb-3" name="comprobante" accept="image/*,application/pdf"
                                    required>
                                <button type="submit" class="btn-buy btn-primary-modern w-100">
            ...
            <div class="product-features pt-3" style="border-top: 1px solid var(--border);">
                <h5 class="features-title"><i class="fas fa-info-circle me-2"></i>Características</h5>
                <ul class="features-list">
                    <?php if (!empty($producto['version'])): ?>
                        <li><i class="fas fa-check-circle"></i> <strong>Versión:</strong>
                            <?php echo htmlspecialchars($producto['version']); ?></li>
                    <?php endif; ?>
                    <?php if (!empty($producto['requisitos'])): ?>
                        <li><i class="fas fa-desktop"></i> <strong>Requisitos:</strong>
                            <?php echo htmlspecialchars($producto['requisitos']); ?></li>
                    <?php endif; ?>
            ...
            <div class="secure-badge mt-3" style="border-top: 1px solid var(--border); padding-top: 15px;">
                <i class="fas fa-shield-alt"></i>
                <span>Transacción segura de PES Bolivia</span>
            </div>
        </div>
    </div>
    ...
    <div class="description-section">
        ...
        <div class="description-text">
            ...
            $desc_limpia = str_replace(array('\\r\\n', '\\n', '\\r', '\r\n', '\n', '\r'), "\n", $desc);
            echo nl2br(htmlspecialchars($desc_limpia));
            ?>
        </div>
    </div>
            ...
            <div class="reviews-summary">
                ...
                <div class="write-review-card">
                    ...
                    <?php if (!empty($success_review)): ?>
                        <div class="alert alert-success p-2 small"><i class="fas fa-check-circle"></i>
                            <?php echo htmlspecialchars($success_review); ?></div>
                    <?php endif; ?>
                    <?php if (!empty($error_review)): ?>
                        <div class="alert alert-danger p-2 small"><i class="fas fa-exclamation-triangle"></i>
                            <?php echo htmlspecialchars($error_review); ?></div>
                    <?php endif; ?>

                    <?php if (!$is_logged): ?>
                        <a href="login.php" class="btn-buy btn-outline-modern"
                    ...
                    <?php else: ?>
                        <button class="btn-buy btn-primary-modern" style="padding: 10px 20px; font-size: 0.9rem;"
                            onclick="document.getElementById('formReviewContainer').style.display='block'; this.style.display='none';">
                            ...
                            <form action="procesar_resena.php" method="POST">
                                ...
                                <div class="mb-3">
                                    <textarea class="form-control" name="comentario" rows="4"
                                        placeholder="Tu opinión sobre el producto..." required></textarea>
                                </div>
                    ...
                    <?php while ($resena = mysqli_fetch_assoc($resenas_list)): ?>
                        <div class="review-card">
                            <div class="review-header">
                                ...
                                <div>
                                    <span
                                        class="review-author"><?php echo htmlspecialchars($resena['usuario_nombre']); ?></span>
                            ...
                            <div class="rating-stars-large" style="margin-bottom: 8px;">
                                <?php for ($i = 1; $i <= 5; $i++): ?>
                                    <i
                                        class="fas fa-star <?php echo $i <= $resena['calificacion'] ? 'stars-filled' : 'stars-empty'; ?>"></i>
                                <?php endfor; ?>
                            </div>
                            <div class="review-date">
                                <?php echo date('d \d\e M \d\e Y', strtotime($resena['fecha_creacion'])); ?></div>
                    ...
                    <?php if ($total_paginas_resenas > 1): ?>
                        <div class="mt-4 text-center">
                            <?php if ($pagina_resenas < $total_paginas_resenas): ?>
                                <a href="producto.php?id=<?php echo $producto['id']; ?>&pagina_resenas=<?php echo $pagina_resenas + 1; ?>#resenas-section"
                                    class="btn-buy btn-outline-modern d-inline-block px-4" style="font-size: 0.9rem;">Ver más
                                    reseñas</a>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
    ...
    <div class="lightbox-overlay" id="lightboxOverlay" onclick="cerrarLightbox(event)">
        ...
        <button class="lightbox-arrow left" onclick="event.stopPropagation(); lightboxNav(-1)"><i
                class="fas fa-chevron-left"></i></button>
        <img src="" alt="" class="lightbox-img" id="lightboxImg" onclick="event.stopPropagation()">
        <button class="lightbox-arrow right" onclick="event.stopPropagation(); lightboxNav(1)"><i
